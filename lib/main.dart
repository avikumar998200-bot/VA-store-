...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void openRealSMS(){
    ...
    showDialog(context: context, builder: (ctx)=> StatefulBuilder(builder: (ctx2,setSt)=> AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, children:[
        Text('8955116739 pe REAL SMS jayega Firebase se', style: TextStyle(fontSize:11, color:Colors.green, fontWeight:FontWeight.bold)),
      ...
      actions:[
        if(!sent) ElevatedButton(onPressed:() async {
          if(num.trim()!='8955116739'){ ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('Only 8955116739'))); return; }
          await FirebaseAuth.instance.verifyPhoneNumber(
            ...
            codeSent: (String vId, int? r){ verId=vId; setSt(()=>sent=true); ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('REAL SMS bhej diya - Inbox dekho'), backgroundColor:Colors.green)); },
  ...
  @override Widget build(BuildContext c){
    return Scaffold(appBar: AppBar(backgroundColor: Colors.white, title: GestureDetector(onLongPressStart: (_)=>startLP(), onLongPressEnd: (_)=>cancelLP(), child: Column(crossAxisAlignment:CrossAxisAlignment.start, children:[ Text('Avira Store', style:TextStyle(color:Color(0xFF9F2089),fontWeight:FontWeight.bold,fontSize:22)), if(isLP) Text('Real SMS ${15-lpCount}s HOLD', style:TextStyle(fontSize:12,color:Colors.red)) ]))), body: GridView.builder(padding:EdgeInsets.all(10), gridDelegate:SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount:2,childAspectRatio:0.7), itemCount:PRODS.length, itemBuilder:(ctx,i){ return Card(child:Column(children:[Expanded(child:Icon(Icons.image,size:50)), Text(PRODS[i].name)])); }));
  }
}
class AdminPanel extends StatelessWidget { @override Widget build(BuildContext c)=> Scaffold(appBar:AppBar(title:Text('ADMIN REAL SMS'), backgroundColor:Colors.red, foregroundColor:Colors.white), body:Center(child:Column(mainAxisAlignment:MainAxisAlignment.center, children:[ Icon(Icons.sms, size:80, color:Colors.green), Text('REAL SMS se khula!', style:TextStyle(fontSize:20, fontWeight:FontWeight.bold)) ]))); }
